public class MoneyInBank implements MoneyTransaction{
    ...
    public MoneyTransaction subtract(MoneyTransaction money) {
        if (this.amount-money.getAmount() < 0) {
            return new MoneyInBank (0, this.interestRate, this.totalLoanAmount);
        }
        return new MoneyInBank (this.amount - money.getAmount(), this.interestRate, this.totalLoanAmount);
    }
    ...
    public void borrow(int loanAmount) {
        this.amount = this.amount + loanAmount;
        this.totalLoanAmount = this.totalLoanAmount + ((loanAmount * (100 + this.interestRate))/100);
    }
    ...
    public void monthlyTransaction() {
        this.amount = this.amount + calcInterest() - totalLoanAmount;
        if (this.amount <0){
            this.amount = 0;
        }
        this.totalLoanAmount = 0;
